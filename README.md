...
Реализация алгоритма: [plug-in](https://github.com/subster2/Rhello/blob/master/plugIn/plugIN.r)

Примеры работы подстановочного алгоритма:

1. Если классы s и t равновероятны и равнозначны, ковариационные матрицы равны, признаки некоррелированы и имеют одинаковые дисперсии

![](https://github.com/subster2/Rhello/blob/master/plugIn/PluginDisp.png)

2. Если признаки коррелированы 

![](https://github.com/subster2/Rhello/blob/master/plugIn/PluginDOntCorre.png)

3. Если ковариационные матрицы не диагональны и не равны, то разделяющая гиперплоскость становится *квадратичной* и "прогибается" так, чтобы менее плотный класс охватывал более плотный.

![](https://github.com/subster2/Rhello/blob/master/plugIn/PlugINElipse.png)

![](https://github.com/subster2/Rhello/blob/master/plugIn/PlugINgiper.png)

![](https://github.com/subster2/Rhello/blob/master/plugIn/PlugINparab.png)
...
• Если длина выборки меньше размерности пространства, **ℓ < n**, или среди
признаков есть линейно зависимые, то матрица **Σ** становится вырожденной. В этом случае обратная матрица не существует и метод вообще неприменим.

• На практике встречаются задачи, в которых признаки «почти линейно зависимы». Тогда матрица **Σ** является плохо обусловленной, то есть близкой к некоторой вырожденной матрице. Это так называемая проблема мультиколлинеарности, которая влечёт неустойчивость обратной матрицы . Они могут непредсказуемо и сильно изменяться при незначительных вариациях исходных данных, например, связанных с погрешностями измерений. Неустойчивость снижает качество классификации.
...
1) Обучение
    ...
    а) Оценивание математических ожиданий ![](http://www.machinelearning.ru/mimetex/?\mu_y)
    
    б) Вычисление общей ковариационной матрицы **Σ** и ее обращение
...
![](https://github.com/subster2/Rhello/blob/master/LDF/LDF.png)

# Линейные	алгоритмы	классификации

Линейные модели широко используются в машинном обучении благодаря их относительной простоте, в некоторых случаях хорошей интерпретируемости и наличию глубоко проработанных численных методов. Простейшим обоснованием линейного классификатора служит его аналогия с нервной клеткой — нейроном. Персептронные принципы обучения, первоначально заимствованные из нейрофизиологии, затем
нашли математические обоснования и с точки зрения градиентных методов минимизации эмпирического риска, и с точки зрения байесовской теории классификации,и с точки зрения статистических оценок обобщающей способности.

Пусть объекты описываются n числовыми признаками  ![](http://www.machinelearning.ru/mimetex/?f_j:\:%20X\to\mathbb{R},\;%20j=1,\ldots,n). Тогда пространство признаковых описаний объектов есть ![](http://www.machinelearning.ru/mimetex/?X=\mathbb{R}^n). Пусть **Y** — конечное множество номеров (имён, меток) классов.

Случай двух классов
Положим **Y={-1,+1}**.
Линейным классификатором называется алгоритм классификации ![](http://www.machinelearning.ru/mimetex/?a:\;%20X\to%20Y) вида

![](http://www.machinelearning.ru/mimetex/?a(x,w)%20=%20\mathrm{sign}\left(%20\sum_{j=1}^n%20w_j%20f_j(x)%20-%20w_0%20\right)%20=%20\mathrm{sign}\langle%20x,w%20\rangle,)
...
Рассматривается некоторая модификация метода стохастического градиента.	Пусть множество ответов алгоритма ![](http://www.machinelearning.ru/mimetex/?$%20Y=\{-1,+1\}$) некоторый объект обучающей выборки ![](http://www.machinelearning.ru/mimetex/?$%20y_i=y_{i}^*(x_i)\in%20Y%20$) — классы, которым принадлежат эти объекты. И пусть алгоритм классификации имеет вид ![](http://www.machinelearning.ru/mimetex/?$a(x,%20w)=sign(\%3Cx,w\%3E)$,) где **<x,w>** — скалярное произведение объекта и его «веса». Таким образом, алгоритм классификации ошибается, если знак этого скалярного произведения не совпал со знаком класса, которому принадлежит объект **x**, то есть если **<x,w>y<0**.
Тогда правило модификации весов примет вид:

если **<x,w>y<0**, то:

![](http://www.machinelearning.ru/mimetex/?$w:=w+\eta%20x%20y$)

Это правило носит название правила Хэбба. Для правила Хэбба справедлива теорема Новикова о сходимости.

Реализация:

![](https://github.com/subster2/Rhello/blob/master/LogistRegr/Hebsrulles.png)
...
![](http://www.machinelearning.ru/mimetex/?w%20\,%20{:=}%20\,%20w%20\,%20-%20\,%20\eta%20\nabla%20Q(w)%20=%20%20w%20-%20%20\eta%20\(%20(w,x)%20-%20y), где ![](http://www.machinelearning.ru/mimetex/?\eta) - темп обучения.

Реализация:

![](https://github.com/subster2/Rhello/blob/master/LogistRegr/AdaLine.png)

## Метод опорных векторов (Support Vector Machines)

Обучение SVM сводится к задаче квадратичного программирования, имеющей единственное решение, которое вычисляется достаточно эффективно даже на выборках в сотни тысяч объектов. Решение обладает свойством разреженности: положение оптимальной разделяющей гиперплоскости зависит лишь от небольшой доли обучающих объектов. Они и называются опорными векторами; остальные объекты фактически не задействуются. С помощью введения функции ядра — метод обобщается на случай нелинейных разделяющих поверхностей.

Пример для линейно	разделимой	выборки
![](https://github.com/subster2/Rhello/blob/master/SVM/SVM2.png)

Пример для линейно	неразделимой	выборки с	малым	числом	объектов,	«мешающих» линейной	отделимости;,	
